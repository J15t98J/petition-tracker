...
exports.trackPetition = functions.https.onRequest((request, response) => {
  ...
  return fetch(`https://petition.parliament.uk/petitions/${
                   request.query.id}.json`)
      .then(response => {
        ...
        if (response.status === 200) {
          ...
          throw new Error(
              `Received ${response.status} for petition #${request.query.id}`);
      })
      .then(json =>
                db.collection('petitions')
                    ...
                    .set({
                      ...
                      responded :
                          json.data.attributes.government_response !== null
                    })
                    .catch(reason => {
                      console.log(reason);
                      responseCode = 500;
...
exports.snapshotPetitions =
    functions.pubsub.schedule('every 5 minutes synchronised')
        .onRun((context) => {
          db.collection('petitions')
              .where("state", "==", "open")
              .get()
              .then(docs => docs.forEach(petition => {
                fetch(`https://petition.parliament.uk/petitions/${
                          petition.id}.json`)
                    .then(response =>
                              response
                                  .json()) // TODO: log 5xx response codes to
                                           // show connection warning on client
                    .then(json => {
                      petition.collection('snapshots')
