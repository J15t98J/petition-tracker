...
exports.trackPetition = functions.https.onRequest((request, response) => {
  ...
  return fetch(
    `https://petition.parliament.uk/petitions/${request.query.id}.json`
  )
    .then((response) => {
...
exports.snapshotPetitions = functions.pubsub
  .schedule("every 5 minutes synchronised")
  .onRun((context) => {
    db.collection("petitions")
      .where("state", "==", "open")
      .get()
      .then((docs) =>
        docs.forEach((petition) => {
          fetch(`https://petition.parliament.uk/petitions/${petition.id}.json`)
            .then((response) => response.json()) // TODO: log 5xx response codes to
